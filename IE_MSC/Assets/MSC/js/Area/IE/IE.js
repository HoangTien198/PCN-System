...
function loadQuillEditor() {
    var Editors = ['#editorBeforeChangeDescription', '#editorAfterChangeDescription', '#editorReason', '#editorCalculateCost','#calculateCostIE'];
...
function btnSaveOnClick() {
    $(document).on('click', '#btn-save-MSC', function () {
        ...
        var calculateCost = $('#editorCalculateCost').val();
                    ...
                    if (beforeChangeDescription.replace(/<(.|\n)*?>/g, '').trim().length === 0) {
                        ...
                        if (afterChangeDescription.replace(/<(.|\n)*?>/g, '').trim().length === 0) {
                            ...
                            if (!reason.replace(/<(.|\n)*?>/g, '').trim().length === 0) {
                                ...
                                data.append("CalculateCost", btoa(unescape(encodeURIComponent(calculateCost))));
...
function btnSaveCalculateOnClick() {
    $(document).on('click', '#btn-save-caculate', function () {
        onload();
        var data = {};
        data.PCNID = mscIDDetailClick;
        var calculateCost = $('#calculateCostIE').val();
        data.CalculateCost = btoa(unescape(encodeURIComponent(calculateCost)));
        $.ajax({
            type: "POST",
            url: "/IE/MSC/SaveMSCCaculateCost",
            ...
            success: function (response) {
                if (response == 1) {
                    ...
                    loadDataTableMSC();
                    mscIDDetailClick = null;
                }
                else {
                    endload();
                    Swal.fire("Server error!", `Update error! Press CTRL + F5 and submit form again or Contact CPD-TE_Auto to support! (Phone: 31746)`, "error");
                }
            },
            error: function (res) {
                endload();
...
function btnDetailMSCOnClick() {
    $(document).on('click', '.btn-detail-msc', function () {
        try {
            onload();
            var mscID = $(this).data('mscid');
            mscIDDetailClick = $(this).data('mscid');
            $.ajax({
                type: "GET",
                url: "/MSC/GetById",
                data: {
                ...
                success: function (response) {
                    ...
                    $('.htmlDecodeCalculateCost').empty(); (jsonResult.CalculateCost) ? $("#calculateCostIE").jqteVal(`${jsonResult.CalculateCost}`) : $("#calculateCostIE").jqteVal("");
                    //$('.okStatusMSC').empty(); (jsonResult.Status >= 4) ? $('.okStatusMSC').append(`Senior Director`) : ``;
                    //$('.okEffectiveDate').empty(); (jsonResult.Status >= 4) ? ((jsonResult.EffectiveDate) ? $('.okEffectiveDate').append(formartDateSQLToyyyyMMdd(jsonResult.EffectiveDate)) : ``) : $('.okEffectiveDate').append(``);
                    ...
                    if (jsonResult.Status == -1 && (jsonResult.BossRejectBy != null)) {
                        for (var ownerStatus of jsonResult.LstOwnerStatus.sort(x => x.Status)) {
                            if (ownerStatus.Status == -1) {//rejected
                                ...
                                $('.reasonReject').empty();
                                $('.reasonReject').append(`<span>${jsonResult.RejectReason}</span></br><span class="text-danger">By Senior Director</span>`);
                                $('.allDepartmentDetail').append(divKy);
                            }
                        }
                    }
                    ...
                    $('.calculateContain').empty();
                    //calculateCost binding button:
                    if (jsonResult.Status == 1) {
                        $('.calculateContain').append(`<button type="button" id="btn-save-caculate" class="btn btn-primary">Save changes</button>`);
                    }
                    // show modal details
